...
EMAIL_ADDRESS = 'anabela.rosanti@ti.ukdw.ac.id'
EMAIL_PASSWORD = 'mtvn rnfa ymro smcj'
...
def send_otp(email, otp):
    ...
    body = f'Kode verifikasi Anda adalah: {otp}'
...
# LOGOUT DAN RATING
# Fungsi untuk membersihkan telegram_id
def clear_telegram_id(telegram_id):
    cursor = db.cursor()
    query = "UPDATE peserta SET telegram_id = NULL WHERE telegram_id = %s"
...
# Memasukkan input pertanyaan user ke database
...
# Cek apakah peserta terdaftar di matakuliah
...
def get_user_info(telegram_id):
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT * FROM peserta WHERE telegram_id = %s", (telegram_id,))
    return cursor.fetchone()
...
def process_nim_step(message):
    ...
    cursor = db.cursor(dictionary=True)
    query = "SELECT email FROM peserta WHERE nim = %s"
    ...
    if result:
        ...
        update_otp(nim, otp)
        send_otp(email, otp)
        msg = bot.reply_to(message, f'Kode verifikasi telah dikirim ke email {email}. Silakan masukkan kode verifikasi tersebut atau ketik /logout untuk keluar.')
...
def process_otp_step(message, nim, chat_id):
    ...
    if result:
        ...
        if stored_otp == otp:
            query = "UPDATE peserta SET telegram_id = %s, otp = NULL, otp_attempts = 0 WHERE nim = %s"
            cursor.execute(query, (chat_id, nim))
            db.commit()
...
# Handler untuk menangani respon rating dari pengguna
...
# Handler untuk pertanyaan pengguna
